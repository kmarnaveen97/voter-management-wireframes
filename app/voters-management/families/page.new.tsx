...
type SortKey = "ward_no" | "house_no" | "mukhiya_name" | "member_count";
...
function QuickStats({ families }: { families: Family[] }) {
  ...
  const stats = [
    { icon: Home, value: families.length, label: "Showing" },
    { icon: Users, value: totalMembers, label: "Members" },
    { icon: BarChart3, value: avgMembers, label: "Avg/House" },
    { icon: TrendingUp, value: largestFamily, label: "Largest" },
  ];
...
function FamiliesLoadingSkeleton() {
  return (
...
export default function HouseholdsPage() {
  ...
  const wards = wardData?.wards?.map((w) => w.ward_no) ?? [];

  // Infinite query for families
  const {
    data,
    isLoading,
    isError,
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const result = await api.getFamilies(pageParam, PAGE_SIZE, selectedListId);
      return result;
    },
    initialPageParam: 1,
  ...
  const allFamilies = useMemo(() => {
    ...
    return families;
  }, [data?.pages, selectedWard, debouncedSearch]);

  // Sort families
  const sortedFamilies = useMemo(() => {
    return [...allFamilies].sort((a, b) => {
  ...
  }, [allFamilies, sortKey, sortDir]);

  // Total count from API
  const totalFamilies = data?.pages?.[0]?.total ?? 0;
  ...
  if (isLoading || listLoading) {
    return <FamiliesLoadingSkeleton />;
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      ...
      {/* Quick Stats */}
      {sortedFamilies.length > 0 && <QuickStats families={sortedFamilies} />}
      ...
      {isError && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <span>
              {error instanceof Error ? error.message : "Unable to fetch households."}
            </span>
            <Button variant="outline" size="sm" onClick={() => refetch()}>
              <RefreshCw className="h-4 w-4 mr-1" />
              Retry
      ...
      {sortedFamilies.length === 0 ? (
        <div className="border rounded-lg p-12 text-center">
          ...
          <p className="text-muted-foreground font-medium">No households found</p>
        ...
        <>
                      ...
                      <TableCell className="hidden md:table-cell">
                        <div className="flex gap-1">
                          {maleCount > 0 && (
                            <Badge
                              variant="outline"
                              className="text-xs bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-800"
                            >
                              {maleCount}M
                            </Badge>
                          )}
                          {femaleCount > 0 && (
                            <Badge
                              variant="outline"
                              className="text-xs bg-pink-50 text-pink-600 border-pink-200 dark:bg-pink-950/30 dark:text-pink-400 dark:border-pink-800"
                            >
                              {femaleCount}F
                            </Badge>
                          )}
          ...
          {!hasNextPage && sortedFamilies.length > 0 && (

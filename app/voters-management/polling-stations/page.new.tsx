...
import {
  usePollingStations,
  useBoothStatistics,
  usePollingStationDetail,
  useBoothVoters,
  usePrefetchBoothVoters,
...
// ============================================================================
// Booth Voters Component (with React Query)
// ============================================================================

function BoothVotersPanel({
  listId,
  booth,
  onExport,
}: {
  listId: number;
  booth: PollingBooth;
  onExport: (booth: PollingBooth, voters: Voter[]) => void;
}) {
  const { data, isLoading, error } = useBoothVoters(listId, booth.polling_booth_id);

  const voters = data?.voters || [];
  const boothDetails = data?.booth;
  const totalVoters = data?.total || 0;
  const sentimentCounts = boothDetails?.sentiment_counts;
  const turnoutCounts = boothDetails?.turnout_counts;

  return (
    <Card>
      <CardHeader className="py-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">Voters in {booth.name}</CardTitle>
          <div className="flex items-center gap-2">
            {voters.length > 0 && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => onExport(booth, voters)}
                className="gap-1.5"
              >
              ...
              <div className="p-4 border-t grid grid-cols-2 gap-4">
                {sentimentCounts && (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(sentimentCounts.support ?? 0) > 0 && (
                        <Badge className="bg-green-100 text-green-700 hover:bg-green-100">
                          Support: {sentimentCounts.support}
                        </Badge>
                      )}
                      {(sentimentCounts.oppose ?? 0) > 0 && (
                        <Badge className="bg-red-100 text-red-700 hover:bg-red-100">
                          Oppose: {sentimentCounts.oppose}
                        </Badge>
                      )}
                      {(sentimentCounts.swing ?? 0) > 0 && (
                        <Badge className="bg-yellow-100 text-yellow-700 hover:bg-yellow-100">
                          Swing: {sentimentCounts.swing}
                        </Badge>
                      )}
                ...
                {turnoutCounts && (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(turnoutCounts.will_vote ?? 0) > 0 && (
                        <Badge className="bg-green-100 text-green-700 hover:bg-green-100">
                          Will Vote: {turnoutCounts.will_vote}
                        </Badge>
                      )}
                      {(turnoutCounts.already_voted ?? 0) > 0 && (
                        <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100">
                          Voted: {turnoutCounts.already_voted}
                        </Badge>
                      )}
                      {(turnoutCounts.wont_vote ?? 0) > 0 && (
                        <Badge className="bg-red-100 text-red-700 hover:bg-red-100">
                          Won't Vote: {turnoutCounts.wont_vote}
                        </Badge>
                      )}
                      {(turnoutCounts.unsure ?? 0) > 0 && (
                        <Badge className="bg-yellow-100 text-yellow-700 hover:bg-yellow-100">
                          Unsure: {turnoutCounts.unsure}
                        </Badge>
                      )}
                      {(turnoutCounts.not_home ?? 0) > 0 && (
                        <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-100">
                          Not Home: {turnoutCounts.not_home}
                        </Badge>
                      )}
                      {(turnoutCounts.needs_transport ?? 0) > 0 && (
                        <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100">
                          Needs Transport: {turnoutCounts.needs_transport}
                        </Badge>
                      )}
                      {(turnoutCounts.unmarked ?? 0) > 0 && (
                        <Badge variant="outline">
                          Unmarked: {turnoutCounts.unmarked}
                        </Badge>
                      )}
                    </div>
                  </div>
...
// ============================================================================
// Station Detail Component (with React Query)
// ============================================================================
...
function StationDetailPanel({
  ...
  listId: number;
  stationCode: string;
  fallbackStation?: PollingStation;
  onClose: () => void;
  onExportVoters: (booth: PollingBooth, voters: Voter[]) => void;
}) {
  const queryClient = useQueryClient();
  const prefetchBoothVoters = usePrefetchBoothVoters();
  ...
  const {
    ...
    refetch,
    isFetching,
  } = usePollingStationDetail(listId, stationCode);
  ...
  // Set first booth as active when data loads
  useEffect(() => {
    if (sortedBooths.length > 0 && !activeBoothTab) {
      setActiveBoothTab(String(sortedBooths[0].polling_booth_id));
    }
  }, [sortedBooths, activeBoothTab]);

  const handleBoothTabChange = (value: string) => {
    setActiveBoothTab(value);
    // Prefetch next booth's voters
    const nextIndex = sortedBooths.findIndex(
      (b) => String(b.polling_booth_id) === value
    );
    if (nextIndex < sortedBooths.length - 1) {
      prefetchBoothVoters(listId, sortedBooths[nextIndex + 1].polling_booth_id);
    }
  };
      ...
      <CardContent className="p-4 space-y-3">
        ...
        {error && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Unable to fetch polling station details. Please try again.
            </AlertDescription>
          </Alert>
        )}
...
export default function PollingStationsPage() {
  ...
  // Auto-scroll to expanded detail
  useEffect(() => {
  ...
  const sortedStations = useMemo(
    ...
    [filteredStations, sortKey, sortDir]
  );

  const boothRowsSorted = useMemo(
    () =>
      [...boothRows].sort((a, b) => {
  ...
  // Stats
  const totalBooths = stations.reduce((sum, s) => sum + (s.booth_count || 0), 0);
  const totalVoters = stations.reduce((sum, s) => sum + (s.voter_count || 0), 0);
  ...
  const loading = stationsLoading || listLoading;
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Button
          ...
          disabled={stationsFetching}
        >
          <RefreshCw className={`h-4 w-4 mr-1 ${stationsFetching ? "animate-spin" : ""}`} />
          Refresh
        </Button>
      </div>
      ...
      <Tabs defaultValue="dashboard" className="w-full">
        ...
        <TabsContent value="dashboard" className="space-y-4 mt-4">
          ...
          {/* Booth Intelligence */}
          {(boothStatsLoading || boothStatsError || boothSummary || boothRows.length > 0) && (
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-base font-semibold">Booth Intelligence</h2>
                  <p className="text-sm text-muted-foreground">
              ...
              {boothStatsError && (
                <Alert variant="destructive">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Unable to fetch booth stats. Please check your API connection.
                  </AlertDescription>
                </Alert>
              )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <Card>
                    <CardContent className="p-3">
                      ...
                      <p className="text-lg font-bold">{boothSummary.total_booths}</p>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="p-3">
                      ...
                      <p className="text-lg font-bold">
                        {boothSummary.total_voters?.toLocaleString?.() ?? boothSummary.total_voters}
                      </p>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="p-3">
                      <p className="text-xs text-muted-foreground">Turnout Coverage</p>
                      <p className="text-lg font-bold">
                        {typeof boothSummary.turnout?.coverage_percent === "number"
                          ? `${boothSummary.turnout.coverage_percent.toFixed(1)}%`
                          : "—"}
                      </p>
                    </CardContent>
                  </Card>
              ...
              {!boothStatsLoading && !boothStatsError && boothRows.length === 0 && (
                <div className="border rounded-lg p-6 text-sm text-muted-foreground">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {boothRowsSorted.map((row, idx) => {
                        ...
                        return (
                          <TableRow
                            key={`${row.polling_booth_id ?? row.booth_id ?? boothLabel}-${idx}`}
                          >
                            <TableCell className="font-medium">{boothLabel}</TableCell>
        ...
        <TabsContent value="polling-stations" className="space-y-4 mt-4">
          ...
          {stationsError && (
            <Alert variant="destructive">
              ...
              <AlertDescription className="flex items-center justify-between">
                <span>Unable to fetch polling stations. Please check your API connection.</span>
          ...
          {loading ? (
            ...
            <div className="border rounded-lg p-12 text-center">
              ...
              <p className="text-muted-foreground font-medium">
                {stations.length === 0 ? "No polling stations found" : "No matching stations"}
              </p>
              ...
              {sortedStations.map((station) => {
                ...
                return (
                  <Card
                    ...
                    onClick={() => setExpandedPsCodeParam(isExpanded ? null : psCodeValue)}
                  >
                    <CardHeader className="pb-2">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <MapPin className={`h-5 w-5 ${isExpanded ? "" : "text-primary"}`} />
                          </div>
                          <div className="min-w-0">
                            <CardTitle className="text-base truncate">
                              {debouncedSearch ? (
                                <HighlightText text={station.name} highlight={debouncedSearch} />
                              ) : (
                                station.name
                              )}
                            </CardTitle>
                            <p className="text-xs text-muted-foreground font-mono">
                              Code:{" "}
                              {debouncedSearch ? (
                                <HighlightText text={station.code} highlight={debouncedSearch} />
                              ) : (
                                station.code
                              )}
                            </p>
                    ...
                    <CardContent className="pt-0 space-y-2">
                      <div className="flex items-center gap-4 text-sm">
                        <div className="flex items-center gap-1.5">
                          <Vote className="h-4 w-4 text-green-600" />
                          <span className="font-semibold">{station.booth_count}</span>
                      ...
                      {station.booth_count > 0 && (
                        <div className="pt-1">
                          <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                            <span>
                              ~{Math.round(station.voter_count / station.booth_count)} voters/booth
                            </span>
                          </div>
                          <Progress
                            value={Math.min(
                              (station.voter_count / (station.booth_count * 500)) * 100,
                              100
                            )}
                            className="h-1"
                          />
          ...
          {expandedPsCode && selectedListId && (
            <div ref={expandedDetailRef} className="scroll-mt-20">
              <StationDetailPanel
                listId={selectedListId}
                stationCode={expandedPsCode}
                fallbackStation={sortedStations.find(
                  (s) => String(s.polling_station_id) === expandedPsCode
                )}
                onClose={() => setExpandedPsCodeParam(null)}
                onExportVoters={exportBoothVoters}
              />
            </div>
          )}
        </TabsContent>

...
function TurnoutStatsCards({
  ...
  const willVoteCount = voters.filter((v) => v.turnout_status === "will_vote").length;
  const wontVoteCount = voters.filter((v) => v.turnout_status === "wont_vote").length;
  const alreadyVotedCount = voters.filter((v) => v.turnout_status === "already_voted").length;
  const unsureCount = voters.filter((v) => v.turnout_status === "unsure").length;
  const notHomeCount = voters.filter((v) => v.turnout_status === "not_home").length;

  const stats = [
    { label: "Will Vote", count: willVoteCount, icon: CheckCircle2, color: "text-green-600" },
    { label: "Already Voted", count: alreadyVotedCount, icon: Check, color: "text-blue-600" },
    { label: "Won't Vote", count: wontVoteCount, icon: XCircle, color: "text-red-600" },
    { label: "Unsure", count: unsureCount, icon: HelpCircle, color: "text-amber-600" },
    { label: "Not Home", count: notHomeCount, icon: HomeIcon, color: "text-gray-600" },
  ];
      ...
      {stats.map(({ label, count, icon: Icon, color }) => (
        <Card key={label}>
          ...
          <CardContent className="text-xs text-muted-foreground">
            {totalVoters > 0 ? ((count / totalVoters) * 100).toFixed(1) : 0}% of total
          </CardContent>
        </Card>
      ))}
    </div>
...
export default function VoterTurnoutPage() {
  ...
  const [bulkNoteStatus, setBulkNoteStatus] = useState<TurnoutStatus | null>(null);
  ...
  const bulkMarkMutation = useMutation({
    ...
    onSuccess: (result, { status }) => {
      toast.success(`${result.data.marked_count} voters marked as ${TURNOUT_LABELS[status]}`);
  ...
  const handleBulkMarkTurnout = useCallback(
    (status: TurnoutStatus, note?: string) => {
      ...
      const voterIds = Array.from(selectedIds)
        .map((id) => {
          const voter = voters.find((v) => String(v.voter_id || v.serial_no) === id);
          return voter?.voter_id || (voter?.serial_no ? parseInt(voter.serial_no, 10) : undefined);
        })
  ...
  const handleQuickMarkTurnout = useCallback(
    (voterId: number, status: TurnoutStatus, note?: string) => {
      quickMarkMutation.mutate({ voterId, status, note });
    },
    [quickMarkMutation]
  );
  ...
  const maleCount = voters.filter(
    (v) => v.gender?.toLowerCase() === "male" || v.gender === "पु" || v.gender?.toLowerCase() === "m"
  ).length;
  const femaleCount = voters.length - maleCount;
  const avgAge =
    voters.length > 0
      ? Math.round(voters.reduce((sum, v) => sum + (v.age || 0), 0) / voters.length)
      : 0;
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {voters.length !== totalVoters && voters.length > 0 && (
              <span className="text-primary"> • Showing {voters.length.toLocaleString()}</span>
            )}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem
                ...
                disabled={computeSentimentsMutation.isPending || !selectedListId}
              >
                <Calculator className="h-4 w-4 mr-2" />
                {computeSentimentsMutation.isPending ? "Computing..." : "Compute Sentiments"}
              </DropdownMenuItem>
      ...
      {isError && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <span>{error instanceof Error ? error.message : "Unable to fetch voters."}</span>
      ...
      {voters.length > 0 && <TurnoutStatsCards voters={voters} totalVoters={totalVoters} />}
        ...
        <Card className="bg-primary/5 border-primary/20">
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-2xl">
              {markedCount.toLocaleString()} / {totalVoters.toLocaleString()} Marked
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {totalVoters > 0 ? ((markedCount / totalVoters) * 100).toFixed(1) : 0}%
                </span>
              </div>
              <div className="h-2 bg-muted rounded-full overflow-hidden">
                <div
                  className="h-full bg-primary transition-all duration-300"
                  style={{ width: `${totalVoters > 0 ? (markedCount / totalVoters) * 100 : 0}%` }}
                />
              </div>
              {selectedIds.size > 0 && (
                <div className="flex gap-2 mt-4 flex-wrap">
                  {(["will_vote", "wont_vote", "unsure", "not_home", "already_voted"] as const).map(
                    (status, idx) => {
                      const icons = {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {bulkNoteStatus && <span className="font-medium">{TURNOUT_LABELS[bulkNoteStatus]}</span>}

...
export default function VoterDetailPage({
  ...
  const [neighborVoters, setNeighborVoters] = useState<{
    prev?: Voter;
    next?: Voter;
  }>({});
    ...
    const fetchData = async () => {
      try {
        ...
        if (voterData.ward_no && voterData.house_no) {
          try {
            const familyData = await api.filterVoters({
        ...
        // Fetch prev/next voters for navigation
        if (voterData.serial_no) {
          try {
            const serialNo = parseInt(voterData.serial_no);
            // Fetch previous voter
            if (serialNo > 1) {
              const prevData = await api.filterVoters({
                list_id: selectedListId,
                per_page: 1,
                ward_no: voterData.ward_no,
              });
              // This is simplified - in real app, fetch by serial_no - 1
            }
          } catch {
  ...
  const handleTagSentiment = useCallback(
      ...
      try {
        const voterIds =
          propagateToFamily && familyMembers.length > 0
            ? [voter.voter_id, ...familyMembers.map((m) => m.voter_id)]
            : [voter.voter_id];
        ...
        if (propagateToFamily && familyMembers.length > 0) {
          ...
          toast.success(
            `Marked ${voterIds.length} people as ${SENTIMENT_CONFIG[sentiment].label}`
          );
        } else {
    ...
    [voter, selectedListId, propagateToFamily, familyMembers]
  );
    ...
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
          ...
          <Card>
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  href={`/families?ward=${voter.ward_no}&house=${voter.house_no}`}
                >
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                {familyMembers.map((member) => {
                  ...
                  return (
                    <Link
                      ...
                      href={`/voters/${member.voter_id || member.serial_no}`}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 print:hidden">
          <Link href={`/voters?ward=${voter.ward_no}`} className="contents">
          ...
          <Link href={`/war-room?ward=${voter.ward_no}`} className="contents">
            <Button variant="outline" className="h-auto py-3 flex-col gap-1">
